...
| [Boris](https://readwithboris.com) | [dergigi/boris-android](https://github.com/dergigi/boris-android) | MIT (© Gigi / dergigi) | `ui/theme/Color.kt` + `Theme.kt` scheme mapping and all six theme variants, `DESIGN.md` token manifest, `MainTab`/`BorisBottomBar` order and icon pairs, `HomeSections.DEFAULT` order and per-section tints, reader meta-chip order and label formats, the 45 %-alpha highlight rule from `HighlightMarks.kt`, settings tree and copy, About carousel copy (verified against a 2026-08-21 recording — `docs/refs/boris/screen-map.md`) | app icon (`public/icons/boris.png` — `res/drawable-nodpi/ic_boris_logo.png` downscaled to 128 px, same as every other client here), UI label strings. The in-app mark (`components/BorisIcons.tsx`) is the pen path from the project's own `boris-logo.svg` on readwithboris.com, no longer a redrawing. That glyph is **Font Awesome Free's `highlighter`** recoloured — measured, not assumed: eleven segment lengths divide into Font Awesome's by 5.7916 with a spread of 0.0001, and the same glyph is the site favicon and `res/drawable/ic_launcher_highlighter.xml`. It ships under **CC BY 4.0, © Fonticons, Inc.** (https://fontawesome.com/license/free), recoloured by Gigi. **Not** the nine `assets/features/*.svg` About illustrations, which are redrawn as abstract stand-ins |
