...
| yak-32 | Profile → Follow / Edit profile | §Profile | dead | Przycisk nie ma `onClick` — spotlight kroku 6 („Follow people") nadal ląduje na kontrolce, której nie da się użyć. **Połowa zamknięta 2026-08-08 (fala 2 tourów):** krok montuje teraz CUDZY profil (`viewProfile` z `payload: 'other'`), więc etykieta brzmi „Follow", a nie „Edit profile" — handler `YakiHonneSimulator.tsx:217-223` wspierał to od dawna, tylko wrapper nie przekazywał payloadu. Do zrobienia został sam `onClick` | `ProfileScreen.tsx:54-56` (wciąż bez handlera) · `YakiHonneSimulatorWithTour.tsx:71` (payload `'other'`) | breaks-showme | S |
