...
| ame-30 | Home → app bar → avatar → **account drawer** | §Account drawer | ok | **Zamknięte 2026-08-06 (wdrożenie FAQ).** Poprzednio: unreachable. Drawer jest wierny (banner, status, „N Following · -- Followers", 11 pozycji menu), ale `setIsDrawerOpen(true)` woła **wyłącznie** `onOpenDrawer` z app-bara — w `switch` komend nie ma nic, a `navigate` przyjmuje tylko id zakładek (payload `'drawer'` zostałby po cichu zignorowany). Do tego **brak `data-tour`** na szufladzie i na jej wierszach → **cała gałąź konto/relaye/klucze jest poza zasięgiem FAQ** | unia: `AmethystSimulator.tsx:27,159-165`; settery: `AmethystSimulator.tsx:216,222,224,226,228`; `components/Drawer.tsx:48` | blocks-showme | S |
