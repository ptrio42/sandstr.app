...
- **Landing** (`onboarding_option_view.dart`): logo mark (tinted `primaryColorDark`) → tagline → **hero illustration** (`initial_onboarding.png` = an iPhone-framed shot of the real OLED-black home feed, orange FAB + action row, in a glassmorphic frame) → **"Login"** (filled) → **"Create account"** (outlined) → EULA line → **"Continue as guest"** (or red "Cancel").
- **Sign-in** (`signin_view.dart`): two segmented options (selected = orange 1.5px border) — **"Keys"** (`keys.svg` → paste npub/nsec/hex; heading "Hey, welcome back"; button toggles "Paste your key" ↔ "Login") and **"Remote signer"** (`share-global.svg` → NIP-46: nostrconnect **QR** + copy + `bunker://` field). Android adds **"Amber"** (external signer). **Sign-in methods: Keys · Remote signer (NIP-46) · Amber.**
- **Create account** (`signup_view.dart`): 5-step wizard (widening progress dots) — Metadata → Packs → Interests/Followings → Wallet → Preview; footer "Next" → "Let's get started"; provisioning shows a Lottie loader + "Initializing account".
...
*(UNVERIFIED: exact English literals for several onboarding i18n keys — `enjoyExpOwnData`, `heyWelcomeBack`, `continueAsGuest`, `letsGetStarted` — read as keys, wording approximate.)*
...
12. **DMSans font + `#171718` near-black dark bg** (not pure OLED by default — OLED `#000000` is a separate "black" theme). Compose is a **bottom sheet** with a **paper-plane** send glyph on an **orange circle** — not an up-arrow.
