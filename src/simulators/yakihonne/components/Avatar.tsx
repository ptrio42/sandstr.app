...
interface AvatarProps {
  seed: string;
  /** Size classes for the container, e.g. "w-11 h-11". */
  className?: string;
  /** Corner rounding — YakiHonne uses rounded-SQUARE avatars in feeds/lists. */
  rounded?: string;
  /** Orange lightning badge (zap-enabled / has a lightning address). */
  zap?: boolean;
}

// CSP-safe, offline, deterministic robohash-style avatar. Shared shape with the
// Amethyst/Damus sims; YakiHonne renders them as rounded squares by default.
export function Avatar({ seed, className = 'w-11 h-11', rounded = 'rounded-[28%]', zap = false }: AvatarProps) {
  ...
  return (
    <div className={`${className} relative shrink-0`}>
      ...
      {zap && (
        <div className="absolute -bottom-1 -right-1 w-[42%] h-[42%] rounded-full bg-[var(--yh-orange)] flex items-center justify-center ring-2 ring-[var(--yh-bg)]">
