...
export const FeedSelector: React.FC<Props> = ({ value, onChange }) => {
  ...
  return (
    <>
      <button className="yakihonne-feedsel" onClick={() => setOpen(true)} data-tour="yakihonne-feedsel">
      ...
      {open && (
        <div className="absolute inset-0 z-[70]" onClick={() => setOpen(false)}>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 pb-2">
              ...
              <button onClick={() => setOpen(false)} className="text-[var(--yh-text-2)]"><XIcon className="w-6 h-6" /></button>
            </div>
            {SOURCES.map((s) => {
