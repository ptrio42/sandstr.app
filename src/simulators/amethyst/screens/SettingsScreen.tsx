...
import {
  BlockedUsersView, HiddenWordsView, MutedThreadsView, SecurityFiltersView, SpammersView,
} from './SecurityFiltersView';
import type { SecuritySection } from './SecurityFiltersView';
...
interface SettingsScreenProps {
  ...
  /**
   * Security Filters state. It lives in `AmethystSimulator` because the note
   * overflow menu's Block and Mute thread fill these lists from outside
   * Settings, exactly as they do upstream.
   */
...
const DETAIL_TITLES: Record<string, string> = {
  ...
  // The four lists v1.13.1 pushed out of the tab strip into screens of their own.
  'security-blocked': 'Blocked Users',
  'security-spammers': 'Spammers',
  'security-hidden': 'Hidden Words',
  'security-muted': 'Muted threads',
  relays: 'Relays',
...
const SECURITY_LISTS = ['security-blocked', 'security-spammers', 'security-hidden', 'security-muted'];

export function SettingsScreen({ onBack, initialSection = null, security }: SettingsScreenProps) {
  const raw = initialSection || 'root';
  const [section, setSection] = useState<string>(DETAIL_TITLES[raw] ? raw : 'root');

  // Multi-select on the three unblockable lists. Upstream this state belongs to
  // the screen and drives the CONTEXTUAL top bar, which is why it is held here
  // and not inside the list view.
  const [selected, setSelected] = useState<string[]>([]);
  const toggleSelected = (id: string) =>
    setSelected((cur) => (cur.includes(id) ? cur.filter((x) => x !== id) : [...cur, id]));
  const openSecurity = (next: SecuritySection) => {
    setSelected([]);
    setSection(next);
  };
  const releaseSelected = () => {
    if (section === 'security-blocked') security.unblockUsers(selected);
    else if (section === 'security-spammers') security.unmarkSpammers(selected);
    else if (section === 'security-hidden') security.showWords(selected);
    setSelected([]);
  };
  const inSelectionMode = selected.length > 0 && SECURITY_LISTS.includes(section);
  ...
  // does upstream; backing out of the root closes Settings entirely. The four
  // blocked-content lists are pushed from Security Filters, so they pop back
  // there rather than skipping a level.
  const back = () => {
    setSelected([]);
    if (section === 'root') onBack?.();
    else if (SECURITY_LISTS.includes(section)) setSection('security');
    else setSection('root');
  };

  return (
    <div className="flex flex-col h-full bg-[var(--md-background)]" data-tour="amethyst-settings">
      {/* `BlockListTopBar`: with a selection live, the whole bar swaps for
          "N selected" + a close button + one Unblock action. */}
      {inSelectionMode ? (
        <div className="md-app-bar md-app-bar-enhanced">
          <button onClick={() => setSelected([])} aria-label="Cancel selection" className="md-app-bar-icon-btn">
            <X className="w-6 h-6 text-[var(--md-on-surface)]" />
          </button>
          <h1 className="flex-1 font-semibold text-[var(--md-on-surface)] px-1">{selected.length} selected</h1>
          <button
            type="button"
      ...
      <div className="flex-1 overflow-y-auto">
        {section === 'root' ? (
          ...
          <SecurityFiltersView state={security} onOpen={openSecurity} />
        ) : section === 'security-blocked' ? (
          <BlockedUsersView state={security} selected={selected} onToggle={toggleSelected} />
        ) : section === 'security-spammers' ? (
          <SpammersView state={security} selected={selected} onToggle={toggleSelected} />
        ) : section === 'security-hidden' ? (
          <HiddenWordsView state={security} selected={selected} onToggle={toggleSelected} />
        ) : section === 'security-muted' ? (
          <MutedThreadsView state={security} />
        ) : section === 'backup-keys' ? (
          <BackupKeysView />
